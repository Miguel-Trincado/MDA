...
export default function EjecutivoView({ db, onSave, onRevisado, embedded }) {
  ...
  if (!nombre) {
    return (
      <div className={embedded ? "" : "max-w-4xl mx-auto px-5 py-16"}>
        <Panel>
          <div className="text-xs text-stone-400 uppercase tracking-wide mb-3">Gestión de cartera</div>
    ...
    <div className={i % 2 === 1 ? "bg-stone-50/50" : "bg-white"}>
      ...
      {expanded && (
        <div className="border-t border-stone-200 px-4 py-4 bg-stone-50/60">
          ...
          <ClientEditForm g={g} onSave={onSave} onRevisado={onRevisado} resetKey={expanded} />
        </div>
      )}
    </div>
  );
